dtmc
//model of ticketing system without any values for variables, so it can be defined by the user through user interface.
...
const int start_zone1; //position zone 1 starts (1-15, and has to be smaller or equal to end of zone 1 and to end of all other zones)
const int end_zone1; //position zone 1 ends (1-15, hast to be bigger or egual to start of zone 1 and has to be smaller than all other zones)
const int start_zone2; //position zone 2 starts (1-15, and has to bigger than end of zone 1)
const int end_zone2; //position zone 2 ends (1-15, and has to be bigger or equal to start of zone 2, bigger than end of zone 1 and smaller than start of zone 3)
const int start_zone3; //position zone 3 starts (1-15, and has to be bigger than end of zone 2)
const int end_zone3; //position zone 3 ends (1-15, has to be bigger or equal to start of zone 3 and bigger than all other zones)

const double phone_fail_input;
const double phone_fail = (phone_fail_input * (end_user-start_user)); //probability user's phone stops working
const double gsm_fail; //probability gsm can't locate phone
const double wifi_fail; //probability wifi can't locate phone
const double gps_fail; //probability gps can't locate phone
const double no_checkin; //probability of user forgetting to check-in 
